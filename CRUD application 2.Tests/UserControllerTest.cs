...
namespace CRUD_application_2.Tests.Controllers
{
    ...
    public class UserControllerTest
    {
        ...
        [Test]
        public void Search_ReturnsCorrectViewWithMatchingUsers()
        {
            // Arrange
            var searchString = "test1";

            // Act
            var result = _controller.Search(searchString) as ViewResult;

            // Assert
            Assert.IsNotNull(result);
            Assert.IsInstanceOf<IEnumerable<User>>(result.Model);

            var matchingUsers = result.Model as IEnumerable<User>;
            Assert.AreEqual(1, matchingUsers.Count());
            Assert.AreEqual("Test User 1", matchingUsers.First().Name);
        }

        [Test]
        public void Search_ReturnsCorrectViewWithNoMatchingUsers()
        {
            // Arrange
